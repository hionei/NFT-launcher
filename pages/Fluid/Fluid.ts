...
import { FluidSimulationConfigProps, activeConfig } from "./types";
...
class webGLFluidEnhanced {
  splats(): void {
    splatStack.push(Math.random() * activeConfig.SPLAT_AMOUNT * 4 + activeConfig.SPLAT_AMOUNT);
  }
  ...
  simulation(canvas: HTMLCanvasElement, config: FluidSimulationConfigProps = {}) {
    Object.assign(activeConfig, config);
    resizeCanvas();
    function ga() {}
    ...
    const baseVertexShader = compileShader(gl.VERTEX_SHADER, SHADERS.BASE_VERTEX);

    const blurVertexShader = compileShader(gl.VERTEX_SHADER, SHADERS.BLUR_VERTEX);

    const blurShader = compileShader(gl.FRAGMENT_SHADER, SHADERS.BLUR_FRAG);

    const copyShader = compileShader(gl.FRAGMENT_SHADER, SHADERS.COPY_FRAG);

    const clearShader = compileShader(gl.FRAGMENT_SHADER, SHADERS.CLEAR_FRG);

    const colorShader = compileShader(gl.FRAGMENT_SHADER, SHADERS.COLOR_FRAG);

    const checkerboardShader = compileShader(gl.FRAGMENT_SHADER, SHADERS.CHECKERBOARD);

    const displayShaderSource = SHADERS.DISPLAY_SHADER_SRC;

    const bloomPrefilterShader = compileShader(gl.FRAGMENT_SHADER, SHADERS.BLOOM_PREFILTER_SHADER_FRAG);

    const bloomBlurShader = compileShader(gl.FRAGMENT_SHADER, SHADERS.BLOOM_BLUR_FRAG);

    const bloomFinalShader = compileShader(gl.FRAGMENT_SHADER, SHADERS.BLOOM_FINAL_FRAG);

    const sunraysMaskShader = compileShader(gl.FRAGMENT_SHADER, SHADERS.SUNRAY_MASK_FRAG);

    const sunraysShader = compileShader(gl.FRAGMENT_SHADER, SHADERS.SUNRAY_FRAG);

    const splatShader = compileShader(gl.FRAGMENT_SHADER, SHADERS.SPLAT_FRAG);

    const advectionShader = compileShader(
      gl.FRAGMENT_SHADER,
      SHADERS.ADVECTION_FRAG,
      ext.supportLinearFiltering ? null : ["MANUAL_FILTERING"]
    );

    const divergenceShader = compileShader(gl.FRAGMENT_SHADER, SHADERS.DIVERGENCE_FRAG);

    const curlShader = compileShader(gl.FRAGMENT_SHADER, SHADERS.CURL_FRAG);

    const vorticityShader = compileShader(gl.FRAGMENT_SHADER, SHADERS.VORTICITY_FRAG);

    const pressureShader = compileShader(gl.FRAGMENT_SHADER, SHADERS.PRESSURE_FRAG);
